    ...
    def fetch_repositories(self):
        ...
        try:
            ...
            for item in repo_items:
                ...
                repo_button.clicked.connect(lambda checked, name=repo_name: self.open_repo(name))
                self.repositories_layout.addWidget(repo_button)

        except requests.RequestException as e:
            QMessageBox.critical(self, "Error", f"Failed to fetch repositories: {e}")

    def open_repo(self, repo_name):
        repo_url = f"https://github.com/Germanized/{repo_name}"
